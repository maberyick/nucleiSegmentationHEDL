...
if utils.load_model(sess, saver, model_out_dir):
    ...
    print('=============================================')
    print(' [*] Load Success!\n')
#names = utils.all_files_under('/mnt/pan/Data7/crb138/dataset/nuclei_ident/tma_image_clustering/train/good/','.png')
names = utils.all_files_under('/scratch/users/crb138/test_img/','.png')
for name in names:
    print(name)
    savingName = '/scratch/users/crb138/test_img/' + os.path.basename(name)[:-4] + '_mask'
    #savingName = '/scratch/users/crb138/epiStroma_tmaYale_qif_GAN/' + os.path.basename(name)[:-4] + '_mask'
    if os.path.isfile(savingName) or os.path.getsize(name)>>20 < 2:
        print('size less than 2MB or file existed , continue to next')
        continue       
        
    #mat_contents = h5py.File(name)
    ...
    #image_sav = Image.fromarray(image)
    #image.save('/scratch/users/crb138/epiStroma_tmaYale_train_good_GAN/test.png')
    #image.save('/scratch/users/crb138/epiStroma_tmaYale_qif_GAN/test.png')
    #mask_dataset_shape = (data.shape[0], 2000, 2000)
    #mask_dataset_shape = (np.shape(image)[0], 2000, 2000)
    ...
    #image = image[...,::-1]
    #image = tf.reshape(image, [-1, 2000, 2000, 3])
    #image = tf.image.resize_image_with_crop_or_pad(image,2000,2000)
    #print(np.shape(image))
    image = np.expand_dims(image, axis=0)
    print(np.shape(image))
    #image = tf.reshape(image, [-1, 2000, 2000, 3])
