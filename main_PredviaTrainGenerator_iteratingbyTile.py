...
#gpuoptions = tf.GPUOptions(allowgrowth=True)
...
if utils.load_model(sess, saver, model_out_dir):
    ...
    print('=============================================')    
    print(' [*] Load Success!\n')
names = utils.all_files_under(r'G:\ccipd\dataset\uh_sclc\tumor_matpatches\\','.mat')
for name in names:
    print(name)
    savingName = r'G:\ccipd\dataset\uh_sclc\tumor_pycellmask\\' + os.path.basename(name)[:-4] + '_mask'
    ...
    try:
        ...
        print('Cannot open file by h5py')
        continue
